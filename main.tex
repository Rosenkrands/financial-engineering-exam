...
\begin{align}
    ...
    \mathscr{R}\left( \mathbf{w} \cdot \mathbf{K}_P \right),
\end{align}
subject to:
\begin{align}
    \sum_{j = 0}^d w_j = 1, \ 
    ...
    \mu \in \mathbb{R},
\end{align}
where $\mathscr{R}$ is a measure of risk and $U(\mathbf{w} \cdot \mathbf{K}_P)$ is the utility of the strategy.
...
\begin{align}
    \mathscr{R}(X) = \sigma(X) = \sqrt{\var(X)} = \sqrt{\mathbb{E}\left[ (X - \mathbb{E}(X))^2 \right]}.
\end{align}
For a portfolio with weights $\mathbf{w}$, we have that
\begin{align}
    \mathscr{R}(\mathbf{w} \cdot \mathbf{K}_P) = \sqrt{\mathbf{w}^\top C \mathbf{w}},
\end{align}
